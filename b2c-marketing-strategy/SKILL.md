...
> **Skill state:** v1.4.0 (visual system unification + animation polish + cleanup).
> Built on v1.3.x (Fast-mode honesty pack + Manrope adoption + stub removal). v1.4 additions: (a) **unified chip system** — `--tone-*` tokens drive all 6 chip families (.tag / .kano / .mkt-prio / .cluster-pill / .dir / .plan) on shared base, semantic mapping standardised across families (high-priority always "crit", positive always "ok", etc.); (b) **table normalisation** — `.head2head` and `.q-table` headers now resolve to the same `--text-mute` colour, font-size, padding as `.apx`; (c) **comm-matrix unification** — hardcoded steel-blue/orange RGBAs replaced with `--tone-info-bg`/`--tone-warn-bg` tokens (theme-adaptive), left fixed column rendered as one continuous bar (was separate rounded rectangles), scroll-hint helper text under the matrix; (d) **smooth animations** — View Transitions API on `setTheme()` for crossfade between themes (Chrome/Edge/Safari 18+, graceful fallback), `interpolate-size: allow-keywords` on `<details>` for smooth expand/collapse; (e) **assumption-num vertical alignment fix** (numbers now centred relative to claim text regardless of line count); (f) **orphan `.add-more` CSS removed**. Builds on v1.2 consolidation. Three additions targeting Fast-mode reader-trust: (a) prominent **fast-mode-banner** rendered under the doc title in Fast runs only — primes the reader before the first chart; (b) **section confidence chips** (`.conf-chip` Verified / Mixed / Mostly hypothesis) next to each `<h3 class="h-sub">` heading — section-level rollup of the per-claim ★/.ext plates; (c) **Mode-aware depth caps** for #consumers, #competitors, #comm-matrix, #mmm, #regions, #comm-plan, #audience-lifecycle, #product-initiatives, #positioning, #business-goal — Fast mode renders skeletal versions of structural sections instead of full Deep-mode depth. Stable across all three modes and three Stage-modifier variants (launch / GTM / concept). English and Russian output. Continues to evolve.
...
First, **load the matched category preset.** Open `references/industry-presets.md` (index, ~30 lines), match the brand to one of the 16 categories, then read **only that category's preset file** in `references/presets/<category>.md` (e.g. `presets/food-delivery.md`). Don't load all presets — the index is enough to route.
...
- **#exec / #basics / #next-steps** — synthesis only. **▼ density rule:** each basics-title (Target audience / Positioning / Points of difference / Communication plan / Budget) MUST carry `<a class="tri" href="#<appendix>">▼</a>` to its appendix anchor; each substantive sub-bullet that names a segment / brand / claim / metric gets its own `<a class="tri" href="...">▼</a>` too. Anchors: #consumers (segments), #competitors (rivals), #positioning (brand pyramid + maps), #brand-health (tracker stats), #comm-plan (channel/lifecycle), #mmm (budget + ROAS), #product-initiatives (flagships), #audience-lifecycle (cohort × stage). Use `<a class="tri" href="#…">` not `<span class="tri">` — the latter is unclickable.
  ...
  - **Body length soft cap.** `#exec` body (everything inside `<section id="exec">` between `<h2>` and `</section>`) targets **≤150 lines of HTML**. `#basics` body targets **≤170 lines**. Above that the synthesis stops being synthesis and becomes a second appendix — readers scroll past instead of using it as an entry point. Two specific length-leashes inside `#exec`:
    - **Structural asymmetries — max 4 (not 5+).** The numbered list inside the market-context block holds the year's distinct strategic moves. Five+ asymmetries is a tell that you're listing observations, not strategic asymmetries. Pick the top 4 that genuinely shape execution; the rest belong in `#assumptions` or `#swot`.
    - **Each intro paragraph stays under 80 words.** Purpose-of-doc + northstar paragraphs each cap at 80 words. If a sentence runs to 30+ words, split it (the existing voice rule already says this — applies double in synthesis).
    Mode 3 Deep is the mode where length sprawl happens; if the run was Deep AND no uploaded docs, the synthesis Bob produces is often padded with restating-the-appendix language. Tighten.
...
- **#brand-health — Y1-target table vs incumbents.** Keep the funnel/attribute table structure. Rename the brand's column header from `[Brand]` (implying current) to `[Brand] (Y1 target)`. Set all of the brand's row values to the Y1 target you'd commit to as a launch ambition (these carry `★` since they're hypotheses by definition). Keep incumbent columns populated with their actual current values from the matched preset's brand-funnel benchmark band or from Research Sweep findings. Add `Delta to leader` column showing the gap. The takeaway block below the table should call out (a) which 1–2 attributes the brand can credibly own in Y1, (b) which gaps will close naturally as the product matures vs require explicit brand investment, (c) tripwires — what attribute level by which quarter would signal the launch story isn't landing. Do NOT apply `.hypothesis-badge` to the section — the table is informative, not a placeholder. The brand-tracker setup recommendation goes into `#next-steps` instead, not into the body of `#brand-health`.
...
- **#business-goal — keep columns, zero out the actual.** The KPI tree's default header is `Metric · Priority · 2025 actual · 2026 target · YoY Δ · Owner`. Keep this structure. Set every `2025 actual` cell to `0%` or `0` (whichever fits the metric type) — pre-launch baseline is genuinely zero, not missing. Treat `2026 target` as Y1 target. The `YoY Δ` column then naturally shows growth-from-zero (e.g. `+35 pp`, `+1.5M users`) — keep `delta.up` arrows. All KPI-tree target numbers carry `★` until first-year actuals exist. In `#references` add a line: "Y1 targets calibrated from [comparable-market source]; revise post first-quarter actuals." Do NOT rename column headers to Y1 / Y2 / Y3 — that's a different document type (3-year plan), and confuses readers expecting standard YoY framing.
